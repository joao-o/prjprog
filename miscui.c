...
const gchar *authors[2] =
  { "João Oliveira (79144)", "Tomás A. Reis (78811)" };

void
about (GtkWidget * widget, progdata * pdat)
{
  GtkWidget *dialog;
  GdkPixbuf *pixbuf;
  ...
  gtk_about_dialog_set_name (GTK_ABOUT_DIALOG (dialog), "Galileo");

  gtk_about_dialog_set_version (GTK_ABOUT_DIALOG (dialog), "versão 1.3");

  gtk_about_dialog_set_authors (GTK_ABOUT_DIALOG (dialog), authors);

  gtk_about_dialog_set_copyright (GTK_ABOUT_DIALOG (dialog), "Grupo 20");
...
colorchange (GtkWidget * widget, progdata * pdat)
{
  ...
  for (i = 0; i < 8; i++)
    {
      gtk_widget_modify_bg (pdat->btn[i], GTK_STATE_NORMAL, &pdat->color[i]);
      gtk_widget_modify_bg (pdat->btn[i],
			    GTK_STATE_PRELIGHT, &pdat->color[i]);
      gtk_widget_modify_bg (pdat->btn[i], GTK_STATE_ACTIVE, &pdat->color[i]);
    }
...
colorresetsing (GtkWidget * widget, progdata * pdat)
{
  if ((GTK_TOGGLE_BUTTON (pdat->btn[0]))->active)
    {
      gdk_color_parse ("#FF8C00", &pdat->color[0]);
      gtk_widget_modify_bg (pdat->btn[0], GTK_STATE_NORMAL, &pdat->color[0]);
      gtk_widget_modify_bg (pdat->btn[0],
			    GTK_STATE_PRELIGHT, &pdat->color[0]);
      gtk_widget_modify_bg (pdat->btn[0], GTK_STATE_ACTIVE, &pdat->color[0]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[1]))->active)
    {
      gdk_color_parse ("#3636FF", &pdat->color[1]);
      gtk_widget_modify_bg (pdat->btn[1], GTK_STATE_NORMAL, &pdat->color[1]);
      gtk_widget_modify_bg (pdat->btn[1],
			    GTK_STATE_PRELIGHT, &pdat->color[1]);
      gtk_widget_modify_bg (pdat->btn[1], GTK_STATE_ACTIVE, &pdat->color[1]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[2]))->active)
    {
      gdk_color_parse ("#00CC33", &pdat->color[2]);
      gtk_widget_modify_bg (pdat->btn[2], GTK_STATE_NORMAL, &pdat->color[2]);
      gtk_widget_modify_bg (pdat->btn[2],
			    GTK_STATE_PRELIGHT, &pdat->color[2]);
      gtk_widget_modify_bg (pdat->btn[2], GTK_STATE_ACTIVE, &pdat->color[2]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[3]))->active)
    {
      gdk_color_parse ("#990931", &pdat->color[3]);
      gtk_widget_modify_bg (pdat->btn[3], GTK_STATE_NORMAL, &pdat->color[3]);
      gtk_widget_modify_bg (pdat->btn[3],
			    GTK_STATE_PRELIGHT, &pdat->color[3]);
      gtk_widget_modify_bg (pdat->btn[3], GTK_STATE_ACTIVE, &pdat->color[3]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[4]))->active)
    {
      gdk_color_parse ("#FFFF00", &pdat->color[4]);
      gtk_widget_modify_bg (pdat->btn[4], GTK_STATE_NORMAL, &pdat->color[4]);
      gtk_widget_modify_bg (pdat->btn[4],
			    GTK_STATE_PRELIGHT, &pdat->color[4]);
      gtk_widget_modify_bg (pdat->btn[4], GTK_STATE_ACTIVE, &pdat->color[4]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[5]))->active)
    {
      gdk_color_parse ("#00B3FF", &pdat->color[5]);
      gtk_widget_modify_bg (pdat->btn[5], GTK_STATE_NORMAL, &pdat->color[5]);
      gtk_widget_modify_bg (pdat->btn[5],
			    GTK_STATE_PRELIGHT, &pdat->color[5]);
      gtk_widget_modify_bg (pdat->btn[5], GTK_STATE_ACTIVE, &pdat->color[5]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[6]))->active)
    {
      gdk_color_parse ("#FFFFFF", &pdat->color[6]);
      gtk_widget_modify_bg (pdat->btn[6], GTK_STATE_NORMAL, &pdat->color[6]);
      gtk_widget_modify_bg (pdat->btn[6],
			    GTK_STATE_PRELIGHT, &pdat->color[6]);
      gtk_widget_modify_bg (pdat->btn[6], GTK_STATE_ACTIVE, &pdat->color[6]);
    }
  else if ((GTK_TOGGLE_BUTTON (pdat->btn[7]))->active)
    {
      gdk_color_parse ("#000000", &pdat->color[7]);
      gtk_widget_modify_bg (pdat->btn[5], GTK_STATE_NORMAL, &pdat->color[7]);
      gtk_widget_modify_bg (pdat->btn[5],
			    GTK_STATE_PRELIGHT, &pdat->color[7]);
      gtk_widget_modify_bg (pdat->btn[5], GTK_STATE_ACTIVE, &pdat->color[7]);
    }
...
{
  GtkWidget *dialog, *mainbox, *btnbox, *colorbox, *reset, *resetsing;
  int i;
